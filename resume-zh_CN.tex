...
\begin{itemize}
  ...
  \item 2021和2023年\textbf{国家奖学金} | 计算机系统竞赛\textbf{国家级一等奖} | 数学建模竞赛\textbf{国家级二等奖} | 华为智能基座奖学金 
\end{itemize}
...
\section{项目经历}
\datedsubsection{\textbf{多模态理解/生成模型的低比特强化学习训练稳定性优化}}{2026.01 - 2026.06}
\textit{科研项目 | Python / CUDA | verl、DanceGRPO 框架}
\begin{itemize}
\item 背景：RL 中 rollout 耗时占比大，低精度推理可加速采样，但训推不匹配会使 ppo\_ratio 偏置、reward 崩溃。
\item 任务：定位并消除训推不匹配引发的LLM及扩散模型 GRPO 训练崩溃，实现低精度 rollout 加速。
\item 工作：\ding{172}在 verl 中实现截断采样与拒绝采样，补充 PPO ratio 均值归一化与梯度裁剪，校正训练与 rollout 的 log-prob 偏移；\ding{173}对 Diffusion GRPO 推导高斯 log-ratio 的 $\Delta$ 分解，定位 reward 崩溃源于 $A\cdot\Delta$ 精度差正反馈漂移，提出零额外耗时的 $\Delta$-debias（居中 ratio）与 $\Delta$-stopgradient（STE 修正梯度方向）；\ding{174}实现 MXFP4 在线 Hadamard 旋转量化（Qutlass kernel）与 MXFP4 QAT 假量化，将离群值能量扩散到块内、抑制 block 量化误差。
\item 结果： LLM/多模态GRPO的MXFP8配置reward接近BF16基线；MXFP4推理经旋转将GSM8K准确率损失由45\%收窄至9\%。
\end{itemize}
